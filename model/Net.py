...
from model.Net_modules import InstanceAdaptiveKeypointDetector, PoseSizeEstimator, \
    NOCS_Predictor, Reconstructor

from HGDG import HierarchicalGeometricDualGraph as GeometricAwareFeatureAggregator
...
class Net(nn.Module):
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        if cfg.rgb_backbone == 'dino':
            print("=> loading DINOv2 model structure from local directory...")
            self.rgb_extractor = torch.hub.load('./dinov2', 'dinov2_vits14', source='local', pretrained=False)
            weights_path = '/workspace/AG-Pose/weights/dinov2_vits14_pretrain.pth'
            if not os.path.exists(weights_path):
                raise FileNotFoundError(f"DINOv2 weights not found: {weights_path}")
            print(f"=> loading DINOv2 weights from local file: {weights_path}")
            self.rgb_extractor.load_state_dict(torch.load(weights_path))
            for param in self.rgb_extractor.parameters():
                param.requires_grad = False
            self.rgb_extractor.eval()
            self.feature_mlp = nn.Sequential(nn.Conv1d(384, 128, 1))
        else:
            raise NotImplementedError("Only DINOv2 backbone is supported.")
    ...
    def forward(self, inputs):
        ...
        if self.cfg.rgb_backbone == 'resnet':
            ...
            dino_feature = F.interpolate(dino_feature, size=(num_patches * 14, num_patches * 14), mode='bilinear',
                                         align_corners=False)
        ...
        fused_feature = self.IAKD.cross_modal_fusion(rgb_local, pts_local)  # (b, 256, n)
        kpt_feature = torch.bmm(heat_map, fused_feature.transpose(1, 2))  # (b, kpt_num, 256)
        kpt_feature = self.GAFA(kpt_feature, kpt_3d.detach(), fused_feature.transpose(1, 2), pts)
        ...
        if self.training:
            ...
            end_points['pred_translation'] = delta_t.squeeze(1) + delta_s * torch.bmm(delta_r, t.unsqueeze(2)).squeeze(
                2) + c.squeeze(1)
...
class Loss(nn.Module):
    ...
    def forward(self, endpoints):
        ...
        # pose
        loss_pose = PoseDis(endpoints['pred_rotation'], endpoints['pred_translation'], endpoints['pred_size'],
                            rotation_gt, translation_gt, size_gt)
        # cd
        loss_cd = self.cd_dis_k2p(pts, pred_kpt_3d)
        # nocs
        kpt_nocs_gt = (pred_kpt_3d - translation_gt.unsqueeze(1)) / (
                    torch.norm(size_gt, dim=1).view(b, 1, 1) + 1e-8) @ rotation_gt
