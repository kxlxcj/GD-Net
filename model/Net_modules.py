...
class CrossModalFusion(nn.Module):
    ...
    def __init__(self, rgb_dim=128, pts_dim=128, out_dim=256):
        super(CrossModalFusion, self).__init__()

        self.conv_rgb_b = nn.Sequential(
            nn.Conv1d(rgb_dim, out_dim, kernel_size=1, bias=False),
            nn.BatchNorm1d(out_dim),
            nn.ReLU(inplace=True)
        )
        self.conv_rgb_a = nn.Sequential(
            nn.Conv1d(out_dim, out_dim, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm1d(out_dim),
            nn.ReLU(inplace=True)
        )

        self.conv_pts = nn.Sequential(
            nn.Conv1d(pts_dim, out_dim, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm1d(out_dim),
            nn.ReLU(inplace=True)
        )

        self.attention_for_rgb = nn.Sequential(
        ...
        self.attention_for_pts = nn.Sequential(
            nn.Conv1d(out_dim * 2, out_dim, kernel_size=3, padding=1, bias=False),  
            nn.BatchNorm1d(out_dim),
            nn.ReLU(inplace=True),
            nn.Conv1d(out_dim, out_dim, kernel_size=3, padding=1, bias=False),  
    ...
    def forward(self, rgb_feat, pts_feat):

        rgb_shallow = self.conv_rgb_b(rgb_feat)  
        rgb_deep = self.conv_rgb_a(rgb_shallow)  

        pts_deep = self.conv_pts(pts_feat)  
        concat_deep = torch.cat([rgb_deep, pts_deep], dim=1)  
        attention_rgb = self.attention_for_rgb(concat_deep)  
        attention_pts = self.attention_for_pts(concat_deep)  

        rgb_gated = rgb_shallow * attention_rgb  

        pts_gated = pts_deep * attention_pts  

        concat_gated = torch.cat([rgb_gated, pts_gated], dim=1)  
      
        fused_feat = self.conv_fuse(concat_gated)  # (B, 256, N)

        return fused_feat


class CrossModalFusion_DualPromptMoE(nn.Module):
    def __init__(
        self,
        rgb_dim=128,
        pts_dim=128,
        ...
        self.conv_pts = nn.Sequential(
            nn.Conv1d(pts_dim, out_dim, kernel_size=1, bias=False),
            nn.BatchNorm1d(out_dim),
            nn.ReLU(inplace=True),
        )
        ...
        gate_hidden = max(32, out_dim // 2)
        self.gate_net = nn.Sequential(
            nn.Conv1d(out_dim * 3, gate_hidden, kernel_size=1, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv1d(gate_hidden, num_experts, kernel_size=1, bias=True),
            nn.Softmax(dim=1),
        )

        self.expert_rgb = self._build_expert(out_dim)
        self.expert_pts = self._build_expert(out_dim)
        self.expert_mix = self._build_expert(out_dim)

        self.rgb_res_proj = nn.Conv1d(out_dim, out_dim, kernel_size=1, bias=True)
        self.pts_res_proj = nn.Conv1d(out_dim, out_dim, kernel_size=1, bias=True)
        self._init_1x1_identity(self.rgb_res_proj)
        self._init_1x1_identity(self.pts_res_proj)

        self.aux_loss = None

    def _init_1x1_identity(self, conv):
        with torch.no_grad():
            conv.weight.zero_()
            conv.weight[:, :, 0].copy_(torch.eye(conv.out_channels, device=conv.weight.device, dtype=conv.weight.dtype))
            if conv.bias is not None:
                conv.bias.zero_()

    def _build_expert(self, dim):
    ...
    def forward(self, rgb_feat, pts_feat):
        ...
        concat_gated = torch.cat([rgb_gated, pts_gated], dim=1)
        fused_base = self.conv_fuse(concat_gated)

        b, c, n = fused_base.shape
        global_ctx = F.adaptive_avg_pool1d(fused_base, 1).expand(-1, -1, n)
        weights = self.gate_net(torch.cat([rgb_shallow, pts_deep, global_ctx], dim=1))

        importance = weights.sum(dim=(0, 2))
        self.aux_loss = self.cv_squared(importance)

        w_rgb = weights[:, 0:1, :]
        w_pts = weights[:, 1:2, :]
        w_mix = weights[:, 2:3, :]

        res_rgb = self.rgb_res_proj(self.expert_rgb(rgb_shallow))
        res_pts = self.pts_res_proj(self.expert_pts(pts_deep))
        res_mix = self.expert_mix(fused_base)

        moe_residual = (res_rgb * w_rgb) + (res_pts * w_pts) + (res_mix * w_mix)
        fused = fused_base + moe_residual
        return F.relu(fused, inplace=False), self.aux_loss
...
class NOCS_Predictor(nn.Module):
    ...
    def forward(self, kpt_feature, index):
        ...
        kpt_nocs = torch.matmul(attn, self.bins_center.to(attn.device)).squeeze(-1)
...
class InstanceAdaptiveKeypointDetector(nn.Module):
    ...
    def forward(self, rgb_local, pts_local):
        """
        前向传播：生成实例自适应的关键点查询和热图

        Args:
            rgb_local: (b, 128, n) - RGB特征
            pts_local: (b, 128, n) - 点云特征

        Returns:
            batch_kpt_query: (b, kpt_num, 256) - 关键点查询特征
            heatmap: (b, kpt_num, n) - 关键点热图
        """
        b, c, n = rgb_local.shape


        input_feature = self.cross_modal_fusion(rgb_local, pts_local)  # (b, 256, n)


        batch_kpt_query = self.kpt_query.unsqueeze(0).repeat(b, 1, 1)  # (b, kpt_num, 256)

        batch_kpt_query, attn = self.attn_layer(batch_kpt_query, input_feature)

        norm1 = torch.norm(batch_kpt_query, p=2, dim=2, keepdim=True)  # (b, kpt_num, 1)
        norm2 = torch.norm(input_feature, p=2, dim=1, keepdim=True)  # (b, 1, n)
